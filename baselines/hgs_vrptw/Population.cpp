...
{
	for (Routes const& routes : warmstartSolutions){
		Individual indiv(params);
		for (size_t i=0; i<routes.size(); i++)
		{
			indiv.chromR[i] = routes[i];
			indiv.chromT.insert(indiv.chromT.end(), routes[i].begin(), routes[i].end());
		}
		indiv.evaluateCompleteCost();
		addIndividual(&indiv, true);
	}	

	if (params->nbClients == 1 && warmstartSolutions.size() == 0)
	{
...
Population::Population(Params* params, Split* split, LocalSearch* localSearch, Solutions const& warmstartSols) : params(params), split(split), localSearch(localSearch), warmstartSolutions(warmstartSols)
{

...
def _get_current_intermediate_result(
    trial: "optuna.trial.FrozenTrial", step: int
) -> float:
    values = np.asarray(list(trial.intermediate_values.values()), dtype=float)
    return values[step]
...
class PercentilePruner(BasePruner):
    ...
    def prune(self, study: "optuna.study.Study", trial: "optuna.trial.FrozenTrial") -> bool:
        ...
        current_intermediate_result = _get_current_intermediate_result(trial, step)
        if math.isnan(current_intermediate_result):
            return True
        ...
        if direction == StudyDirection.MAXIMIZE:
            return current_intermediate_result < p
        return current_intermediate_result > p

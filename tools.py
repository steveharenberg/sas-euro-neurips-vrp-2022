...
def populate_jobs(instance, jobs, priorities): 
    demands = instance['demands']
    is_depot = instance['is_depot']
    coords = instance['coords']
...
def populate_vrptw(instance, name="problem", steps=None, nvehicles=None, priorities=None): 
    meta = {}
    coords = instance['coords']
    jobs = []
    populate_jobs(instance, jobs, priorities)
    ...
    if "VEHICLES" not in meta:
        meta["VEHICLES"] = len(jobs) if nvehicles is None else nvehicles
        ...
        vehicles.append(
            {
                ...
                "end_index": 0,
                "capacity": [capacity],
                "time_window": [int(x) for x in instance['time_windows'][0]],
                "steps": route,
            }
        )
...
def write_json(filename, instance, name="problem", steps=None, nvehicles=None, priorities=None):   
    demands = instance['demands']
    is_depot = instance['is_depot']
    duration_matrix = instance['duration_matrix']
    assert (np.diag(duration_matrix) == 0).all()
    ...
    with open(filename, 'w') as f:
        json.dump(populate_vrptw(instance, name=name, steps=steps, nvehicles=nvehicles, priorities=priorities), f)


def run_vroom(instance, tmp_dir, time_limit, explore_level, init_routes=[], nvehicles=None, priorities=None):
    start_time = time.time()
    solutions = []
    executable = os.path.join('baselines', 'vroom', 'bin', 'vroom')
    instance_filename_json = os.path.join(tmp_dir, "problem_vroom.json")
    write_json(instance_filename_json, instance, steps=init_routes, nvehicles=nvehicles, priorities=priorities)
